// app.js - ES Module entry (expects supporting modules in ./modules/)
// If modules are missing, app degrades gracefully and logs helpful messages.
...
/* ---------- Data (small sample; extend in modules or data file) ---------- */
const songs = [
...
const modalSongProgress = qs("#modal-song-progress");
...
function openSongModal(song) {
  ...
  (song.chords || []).forEach(ch => {
    ...
    pill.addEventListener("click", () => {
      ...
      // switch to chords section for deeper view
  ...
  // render modal fretboard if available
  const modalCanvas = qs("#modal-fretboard-canvas");
  const modalRoot = (song.chords && song.chords[0]) ? song.chords[0].replace(/[^A-G#b]/g,"") : "C";
  if (modalCanvas) {
    try { renderFretboard(modalCanvas, modalRoot, "pentatonic-minor", "none", 12); }
    catch (err) { console.warn("renderFretboard failed", err); }
  }
}
...
fbRenderBtn.addEventListener("click", () => {
  ...
  try { renderFretboard(fbCanvas, root, scaleType, overlayType, maxFret); }
  catch (err) { console.warn("renderFretboard failed", err); }
});

qs("#modal-fb-render-btn")?.addEventListener("click", () => {
  if (!currentModalSong) return;
  const overlayType = qs("#modal-fb-overlay-select").value;
  const root = (currentModalSong.chords[0] || "C").replace(/[^A-G#b]/g,"");
  try { renderFretboard(qs("#modal-fretboard-canvas"), root, "pentatonic-minor", overlayType, 12); }
  catch (err) { console.warn("renderFretboard failed", err); }
});
...
function init() {
  renderSongs();
  renderChordList();
  try { buildPatternDots(normalizePatternValue(patternSelect.value), patternVisual); }
  catch (err) { /* ignore */ }
  try { renderFretboard(fbCanvas, fbRootSelect.value, fbScaleSelect.value, "none", parseInt(fbMaxFretInput.value,10) || 12); }
  catch (err) { /* ignore */ }
